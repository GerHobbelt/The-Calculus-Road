...
function convertFloatToFraction(value, largestDenominator) {
  ...
  let a = 0,
      ...
      c = 1,
      d = 1;
  let integerPart = Math.round(value); // calc remainder:
  ...
  largestDenominator++;
  console.log("Test FloatToFraction:", {
    integerPart,
    fracValue,
    largestDenominator
  ...
  for (let i = 0; i < loopLimit; i++) {
    ...
    if (medianDenominator >= largestDenominator || IEEE754CalcPrecLimitReached) {
      let m1 = a / b;
      let m2 = c / d; // console.log("hitting denominator limits:", fracValue - m1, m2 - fracValue, (m2 + m1) / 2, (m2 + m1) / 2 - fracValue);
      ...
      if (m2 < fracValue) {
        ...
        for (let expectLastRound = loopLimit; expectLastRound !== 0; expectLastRound--) {
          ...
          do {
            ...
            m2 = mn / md; // console.log(`R-count`, count, multiplier, expectLastRound, m2 - fracValue, mn - a, md - b);
        ...
        rCount += mult + 1;
        rMsg = `${mn} / ${md} <> ${c} / ${d}`; // Check if we hit the IEEE754 calculus precision limits:
      ...
      if (m2 > fracValue) {
        ...
        for (let expectLastRound = loopLimit; expectLastRound !== 0; expectLastRound--) {
          ...
          do {
            ...
            m2 = mn / md; // console.log(`L-count`, count, multiplier, expectLastRound, m2 - fracValue, mn - c, md - d);
          } while (m2 > fracValue && md < largestDenominator);
        ...
        lCount += mult + 1;
        lMsg = `${a} / ${b} <> ${mn} / ${md}`; // Check if we hit the IEEE754 calculus precision limits:
...
//# sourceMappingURL=IEEE754-floatingpoint-number-to-fraction.js.map