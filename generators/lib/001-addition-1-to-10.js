...
function convertFloatToFraction(value, largestDenominator) {
  let a = 0,
      ...
      c = 1,
      d = 0;
  largestDenominator = largestDenominator || Infinity; // Because we don't want to run into fruitless long inner loop runs (see further below),
  ...
  // an Infinite-sized denominator in there:

  largestDenominator++;
  console.log("Test FloatToFraction:", value, largestDenominator); // The achievable accuracy of the conversion is *not* 0.5/largestDenominator
  ...
  const loopLimit = 1e6; // use a 'ridiculous' loop limit, so that we **will** terminate in 
  ...
  for (let i = 0; i < loopLimit; i++) {
    ...
    if (medianDenominator >= largestDenominator || IEEE754CalcPrecLimitReached) {
      let m1 = a / b;
      let m2 = c / d; // console.log("hitting denominator limits:", value - m1, m2 - value, (m2 + m1) / 2, (m2 + m1) / 2 - value);
    ...
    let median = medianNumerator / medianDenominator; // console.log(`iterator ${i}: diff: ${diff}, a: ${a}, b: ${b}, c: ${c}, d: ${d}`);
      ...
      if (m2 < value) {
        ...
        for (let expectLastRound = loopLimit; expectLastRound !== 0; expectLastRound--) {
          ...
          do {
            ...
            m2 = mn / md; // console.log(`R-count`, count, multiplier, expectLastRound, m2 - value, mn - a, md - b);
        ...
        rCount += mult + 1;
        rMsg = `${mn} / ${md} <> ${c} / ${d}`; // Check if we hit the IEE754 calculus precision limits:
      ...
      if (m2 > value) {
        ...
        for (let expectLastRound = loopLimit; expectLastRound !== 0; expectLastRound--) {
          ...
          do {
            ...
            m2 = mn / md; // console.log(`L-count`, count, multiplier, expectLastRound, m2 - value, mn - c, md - d);
          } while (m2 > value && md < largestDenominator);
        ...
        lCount += mult + 1;
        lMsg = `${a} / ${b} <> ${mn} / ${md}`; // Check if we hit the IEE754 calculus precision limits:
...
let randSeed = 42; // direct implementation of fastrand()
...
if (1) {
  ...
  convertFloatToFraction(1.0000000001); // Testing a mantissa edge case:

  convertFloatToFraction(1.000000000000001); // More tests...

  convertFloatToFraction(0.20001); // test APPROXIMATE: close enough...
  ...
  convertFloatToFraction(3.456789, 1e3); // test =APPROXIMATE=: smack dab in the middle:

  convertFloatToFraction(4.012779683161849, 1e2); // test faulty and edge-case inputs for stability/termination of the run:

  convertFloatToFraction(1e-300);
  convertFloatToFraction(1e300);
  convertFloatToFraction(Infinity);
  convertFloatToFraction(NaN);
  console.log("totals", totals, power, altPower); // console.log((randFloat2(1e6)+1e6)|0);
}
